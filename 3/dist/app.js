...
const solveForElves = (elvesWithSacks) => elvesWithSacks.map((sacks) => sacks.map((items) => new Set(items))
    .reduce((sack, otherSack) => new Set([...sack].filter(item => otherSack.has(item)))))
    .map((sacks) => [...sacks][0])
    .map((item) => (item.charCodeAt(0) > 90) ? item.charCodeAt(0) - 96 : item.charCodeAt(0) - 38)
